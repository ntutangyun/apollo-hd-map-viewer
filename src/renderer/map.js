...
export default class Map {
    ...
    addJunctionId(junction, coordinates, scene) {
        ...
        const text = this.textRender.drawText(
            junction.id.id, scene, colorMapping.WHITE, TEXT_ALIGN.LEFT);
        if (text) {
            ...
            for (let i = 0; i < newData[kind].length; ++i) {
                switch (kind) {
                    ...
                    case "crosswalk":
                        ...
                        break;
                    case "junction":
                        const junction = newData[kind][i];
                        if (junction.type === "DEAD_END") {
                            deadEndJunction.push(junction);
                        }

                        this.data[kind].push(Object.assign(junction, {
                            drewObjects: this.addBorder(
                                junction, colorMapping.BLUE, coordinates, scene,
                            ),
                            text: this.addJunctionId(junction, coordinates, scene),
                        }));
                        break;
                    case "pncJunction":
                        this.data[kind].push(Object.assign(newData[kind][i], {
